...
def load(filename, start=None, end=None, order=None, world=False):
    ...
    for line in f:
        ...
        if dmatch:
            ...
            '''
            assert len(names) == len(j_channels)
            cur_p = 0
            for j, nc in enumerate(j_channels):
                if nc == 3:
                    rotations[fi, j] = data_block[cur_p:cur_p + 3]
                elif nc == 6:
                    positions[fi, j] = data_block[cur_p:cur_p + 3]
                    rotations[fi, j] = data_block[cur_p + 3:cur_p + 6]
                else:
                    raise Exception("Too many channels! %i" % channels)
                cur_p += nc
            i += 1
...
if __name__ == '__main__':
    path = '/home1/wurundi/code/cvpr2018nkn/datasets/test/new_motion/Granny/Catwalk Idle To Walk Forward.bvh'
    # path = '/home1/wurundi/code/hmr/assets/penn0001.bvh'
    anim = load(path)[0]
